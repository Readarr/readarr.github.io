function scrollToHash(hash, speed) {
    if (hash.startsWith("#downloads-v1-"))
        hash = "#downloads-v1";
...
function openTab(hash) {
    if (hash.startsWith('#downloads-v1-linux-'))
        $('a.nav-link[href=#downloads-v1-linux]').tab('show');
    if (hash.startsWith('#downloads-v1-nas-'))
        $('a.nav-link[href=#downloads-v1-nas]').tab('show');
